...
def main(args, results_dir):
    ...
    train_data, val_data, _ = load_data(args.data_dir, args.region_name, args.target)

    logger.info("Building model...")
    model = build_model(train_data, config, results_dir, device, args.project, args.target)
    ...
    logger.success(
        f"✅ Final model trained and saved to {os.path.join(results_dir, 'checkpoints')}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--region_name", type=str, default="promoters_1024bp", help="Region name")
    parser.add_argument("--target", type=str, default="MLL-N", help="Target column name")
    parser.add_argument("--project", type=str, default="SEM_MLL-N_TF", help="WandB project name")
    parser.add_argument("--model_name", type=str, default="GANDALF_SEM", help="Model name")
    parser.add_argument("--best_config_path", type=str, required=True, help="Path to best_run_config.json from sweep")
